...
public class MyJWTUtils {
    ...
    public void verifyWithPublicKey(String token, String publicKey) {
        try {
            ...
            if (!flag) {
                throw new OntFsException("Token verify", ErrorInfo.VERIFY_TOKEN_FAILED.descCN(), ErrorInfo.VERIFY_TOKEN_FAILED.descEN(), ErrorInfo.VERIFY_TOKEN_FAILED.code());
            }

            if (jwt.getExpiresAt().before(new Date())) {
                throw new OntFsException("Token verify", ErrorInfo.TOKEN_EXPIRED.descCN(), ErrorInfo.TOKEN_EXPIRED.descEN(), ErrorInfo.TOKEN_EXPIRED.code());
            }
        } catch (JWTDecodeException e) {
            e.printStackTrace();
            throw new OntFsException("解密Token中的公共信息出现JWTDecodeException异常");
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void verifyAccessToken(String token) {
        if (!getContentType(token).equals(Constant.ACCESS_TOKEN)) {
            throw new OntFsException("verify token", ErrorInfo.TOKEN_TYPE_ERROR.descCN(), ErrorInfo.TOKEN_TYPE_ERROR.descEN(), ErrorInfo.TOKEN_TYPE_ERROR.code());
        }
        verify(token);
    }
...
//            throw new CustomException("解密Token中的公共信息出现JWTDecodeException异常");
    ...
    public String getContentType(String token) {
        try {
            if (token == null) {
                throw new OntFsException("token为空");
            }
            DecodedJWT jwt = JWT.decode(token);
            // 只能输出String类型，如果是其他类型返回null
    ...
    public String signAccess(String aud, String dataId) {
        ...
        return MyJwt.create().withIssuer(configParam.JWT_ISSUER_ONTID).withExpiresAt(new Date(new Date().getTime() + Constant.ACCESS_TOKEN_EXPIRE)).withAudience(aud).withIssuedAt(new Date()).
                withJWTId(UUID.randomUUID().toString().replace("-", "")).withClaim("content", contentData).sign(configParam.JWT_ISSUER_PRIVATEKEY);
    }
